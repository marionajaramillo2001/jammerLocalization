...
# flags
continual_learn = 1 # 0: at each time t the model is not updated, rather retrained from scratch
...
pass_last_weights = 0 # pass last time instant weights or generate random weights at each training time
plot_model_field = 0 # plot the trained model probing it with a sample grid (plot only every X MC and every X time)
...
lam2 = 1 # weight coeff for weight rate term
reg_weights_rate_type = 'time_instants'
# reg_weights_rate_type = 'fixed'
fixed_weight_rate = 0.02
reg_pred_jloc = 0 # regularization based on predicted Jloc at t from t-1 (true Jloc(t) or KF)
lam3 = 200 # reg coefficient
reg_pred_P0 = 0 # regularization based on predicted P0 at time t (linear extrapolation from P0 at t-1?). Needs a model able to learn a correct P0
lam4 = 200 # reg coefficient
...
Nmc =10 # no. of Monte Carlo simulations runs
...
meas_noise_var_iter = np.array([0.001])#[10, 10/np.sqrt(10),  1, 0.1, 0.01]) #, 0.001])  # noise variance affecting power measurements
...
# erase neginf from obstructed signal (workaraound)
min_no_inf = train_y_original[torch.logical_not(train_y_original.isneginf())].min()
train_y_original[train_y_original.isneginf()] = min_no_inf-80
min_no_inf = test_y_original[torch.logical_not(test_y_original.isneginf())].min()
test_y_original[test_y_original.isneginf()] = min_no_inf-80
...
for ii_snr in range(N_snr):
    ...
    for n in iter_list:
        ...
        for t in range(T):
            ...
            #%% Data down-selection 
            if data_selection==1 or(data_selection==2 and t==0):
                ...
                # data select to not normalized (just for MLE)
                train_y_nonorm_select = train_y_nonorm.detach().clone()
                
                # no need to change train_y,test_y,train_x,test_x
            ...
            if t==0 or not(continual_learn):
                ...
                # use a Jloc prediction for regularization
                #Jloc_pred = KF(Jloc_vec[t-1,:,n])
                Jloc_pred = trueJloc[t,:] #validation purpose
                
                
                # every_epoch, model_NN will update
                train_mse, test_mse, model_NN,loss_parts,param_old, model_NN_old = model_nn.continual_learn(train_x,
                                                                         train_y.reshape(-1,1),
                                                                         test_x,
                                                                         test_y.reshape(-1,1),
                                                                         model_NN,epochs,lr,
                ...
                #param_rate, DeltaT = f.estimate_param_rate(reg_weights_rate_type,model_NN.get_NN_param(),param_old,fixed_weight_rate)
                param_rate, DeltaT = f.estimate_param_rate2(reg_weights_rate_type,model_NN,model_NN_old,fixed_weight_rate)
                ...
                loss_part4.extend(loss_parts[3])
            ...
            #%% Plot model
            if plot_model_field and n%10==0 and t%10==0:
                test_y_predict = model_NN(train_x).detach()
                f.plot_field(test_y_predict,train_x,model_NN,trueJloc)
            ...
            if aggregate_over_time:
                if t == 0:
                    ...
                    loss_part1_tot = np.array(loss_part1); loss_part2_tot = np.array(loss_part2); loss_part3_tot = np.array(loss_part3); loss_part4_tot = np.array(loss_part4)
        ...
        if aggregate_over_time:
            train_loss = train_loss_tot/T; test_loss = test_loss_tot/T
            rmse_vec = rmse_vec_tot/T; rmse_vec2 = rmse_vec2_tot/T
            loss_part1 = loss_part1_tot/T; loss_part2 = loss_part2_tot/T; loss_part3 = loss_part3_tot/T; loss_part4 = loss_part4_tot/T
    ...
    loss_part1 = loss_part1_tot/Nmc; loss_part2 = loss_part2_tot/Nmc; loss_part3 = loss_part3_tot/Nmc; loss_part4 = loss_part4_tot/Nmc
...
if continual_learn:
    ...
    if loss_part4.shape[0]!=0:
        tmp = len(loss_part1)-len(loss_part4)
        plt.plot(range(epochs_init+tmp,epochs_init+len(loss_part1)),loss_part4,label='reg est. jam. pos.')
