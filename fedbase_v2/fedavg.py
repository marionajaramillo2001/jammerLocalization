...
class FedAvg(object):
    def __init__(self, model, optimizer_nn, optimizer_theta, algorithm, mu, local_epochs, num_rounds,weight_ratio='sample_power',min_value=0.0, max_value=100., betas=None):
    ...
    def model_update(self, dataloader, y_mean=None, round=None):
    ...
    def train(self, data_splited, real_loc):
        ...
        train_loaders, test_loader_global,max_values_each_partition, train_y_mean_splited,  train_x_max, train_y_max = data_splited
        ...
        for round in range(self.num_rounds):
            model_list = []
            for i in range(len(train_loaders)):
                model, loss = self.model_update(train_loaders[i], train_y_mean_splited[i],round)
