...
disrupt_pos = 0; %0 no dsrpt, 1 disruption, 2 partial random disruption (half are not disrupted)
loadMeasurements = 0;     % avoid recomputing raytracing to disrupt data
N0 = -204.4; % dBW-Hz
cn0_nom = 45; % dBHz (45 for pathloss, 40 for urban)
rx_sens = 21; % dBHz
deltaCN0method = 'model1'; % else use a rough scaling coefficient
posEstVar = 0; % agents' position estimation error variance (m^2)
...
% compute threshold
JN0 = P_jam_rx-N0;
DeltaCN0 = deltaCN0givenJN0(JN0,deltaCN0method);
...
function DeltaCN0 = deltaCN0givenJN0(jn0,deltaCN0method)
if deltaCN0method == 'model1'
    deltaCN0vec = [-0.7 -0.6 -0.5  -0.7 -0.6 -0.5 -0.6 -0.8  -1.2 -1.5 -2.5 -3.0 -4  -6 -7 -9 -12 -14];
    JN0vec      = [55:2:89];
    DeltaCN0 = interp1(JN0vec,deltaCN0vec,jn0,'linear','extrap');
    %figure,plot([JN0vec jn0(1)],[deltaCN0vec DeltaCN0(1)],'-o')
