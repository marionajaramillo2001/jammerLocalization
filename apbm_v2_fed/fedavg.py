...
class FedAvg(object):
    def __init__(self, model_init, model, optimizer_nn, optimizer_theta, optimizer_P0, optimizer_gamma, batch_size, local_epochs_nn, local_epochs_pl, local_epochs_apbm, num_rounds_nn, num_rounds_pl, num_rounds_apbm):
        """
        ...
        self.server_model_init = model_init
    ...
    def model_update(self, dataloader, training_phase):
        ...
        if training_phase == 'NN':
            model_init = copy.deepcopy(self.server_model_init)
    ...
    def server_update(self, model_list, uploaded_weights, training_phase):
        ...
        if training_phase == 'NN':
            with torch.no_grad():
                for name, param in self.server_model_init.named_parameters():
                    param.data.zero_()
            for w, client_model_init in zip(uploaded_weights, model_list):
                for server_nn_param, client_nn_param in zip(self.server_model_init.parameters(), client_model_init.parameters()):
                    server_nn_param.data += client_nn_param.data * w
            return self.server_model_init
