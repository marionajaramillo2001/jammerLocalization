...
def main_fl(train_x,train_y,test_x,test_y,layer_wid,nonlinearity,epochs,lr,data_max,data_min,\
         grid_size=(2,3),weight_ratio='power',algorithm='FedAvg',\
            mu=0.1,local_epochs=1, model_mode='both',split='random'):
    '''
    Main function for training the hybrid model with Federated Learning framework
    '''
    ...
    gamma = 2
    model_aug = Net_augmented(input_dim, layer_wid, nonlinearity,gamma,theta0,data_max,data_min,model_mode)
    # send the model to the device
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model_aug.to(device)
    # use dot Notation to call the class
    # grid_size = (2,3)
    # split = 'area_grid' # 'area_grid','point', random
    power_ratio = 'exp' #exp
    if split == 'point':
        num_clients = len(train_data)
    elif split == 'area_grid':
        num_clients = grid_size[0]*grid_size[1]
    elif split == '2points':
        num_clients = 50
    else:
        num_clients = 20
    args = {'model': model_aug, 'lr': lr, 'local_epochs':local_epochs,
        'num_rounds':epochs, 'train_data': train_data, 'test_data': test_data,
        'num_clients':num_clients, 'local_bs':40, 'split': split,#'area_grid',
        'grid_size':grid_size,'weight_ratio':weight_ratio,\
            'power_ratio':power_ratio, 'algorithm':algorithm,'mu':mu,'num_workers':8}
    # https://stackoverflow.com/questions/16279212/how-to-use-dot-notation-for-dict-in-python
    args = convert_to_dot_notation(args)
    fedavg = FedAvg(args)
    train_loss, test_loss, model_aug = fedavg.train()

    return train_loss, test_loss, model_aug
...
def main(train_x,train_y,test_x,test_y,layer_wid,nonlinearity,epochs,lr,data_max,data_min):
    ...
    # define main function, return the train loss and test loss
def continual_learn(train_x,train_y,test_x,test_y,model_NN,epochs,lr,
                    reg_flag=0,pass_last_weights=1,reg_weights_rate=0,*args):
    """
    layer_wid: NN strucutre. [128,64,output_dimension]
    nonlinearity: activation function. 'relu'
    epochs: training epochs
    ...
    if not(pass_last_weights):
        layers = model_NN.get_layers()    
        nonlinearity = args[2]
        model_NN = Net(layers[0], layers[1:], nonlinearity)
    ...
    for epoch in range(epochs):
        ...
        train_mse[epoch] = train_loss
        # test loss
        y_predict_test = model_NN(test_x)
        test_loss = loss_function(y_predict_test, test_y)
        if reg_flag:
            param = model_NN.get_param()
    ...
    return train_mse, test_mse, model_NN, loss_parts, param_old
...
if __name__ == "__main__":
    ...
    for i in range(times):
        ...
        print('epoch: ',i,'train_mse: ',train_mse[-1],'test_mse: ',test_mse[-1])
