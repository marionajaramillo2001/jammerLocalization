...
class FedAvg(object):
    """
    Federated Averaging (FedAvg) implementation for updating a global model 
    across multiple clients using different training strategies (NN, PL, APBM).

    Attributes:
    - server_model (nn.Module): The global model maintained by the server.
    - optimizer_nn, optimizer_theta, optimizer_P0, optimizer_gamma: Optimizers for updating model parameters.
    - local_epochs_nn, local_epochs_pl, local_epochs_apbm: Number of local epochs for each training phase.
    - batch_size (int): Batch size used in training.
    - num_rounds_nn, num_rounds_pl, num_rounds_apbm: Number of training rounds for each phase.
    - weights_nn_type, weights_pl_type, weights_apbm_type: Weighting methods for loss computation.
    """
    ...
    def __init__(self, model, optimizer_nn, optimizer_theta, optimizer_P0, optimizer_gamma, batch_size, local_epochs_nn, local_epochs_pl, local_epochs_apbm, num_rounds_nn, num_rounds_pl, num_rounds_apbm):
        """
        Initializes the FedAvg class, setting up the global model, optimizers, and training configurations.

        Parameters:
        - model (nn.Module): The global model.
        - optimizer_nn, optimizer_theta, optimizer_P0, optimizer_gamma: Optimizers for different parameters.
        - batch_size (int): Batch size for training.
        - local_epochs_nn, local_epochs_pl, local_epochs_apbm (int): Number of local training epochs for each phase.
        - num_rounds_nn, num_rounds_pl, num_rounds_apbm (int): Number of federated training rounds for each phase.

        Returns:
        - Initializes the class attributes and sets up the federated learning environment.
        """
    ...
    def compute_loss_nn(self, output, target, weights):
        """
        Computes the weighted loss for the Neural Network (NN) phase.

        Parameters:
        - output (torch.Tensor): Model predictions.
        - target (torch.Tensor): Ground truth labels.
        - weights (torch.Tensor): Sample weights for loss computation.

        Returns:
        - loss (torch.Tensor): Computed weighted MSE loss.
        """
    ...
    def compute_loss_pl(self, output, target, weights):
        """
        Computes the weighted loss for the Pathloss Model (PL) phase.
    ...
    def compute_loss_apbm(self, output, target, weights):
        """
        Computes the weighted loss for the Augmented Physics-Based Model (APBM) phase.
    ...
    def model_update(self, dataloader, training_phase):
        """
        Performs local training for a given training phase (NN, PL, APBM) on client data.

        Parameters:
        - dataloader (DataLoader): DataLoader containing local training data.
        - training_phase (str): The training phase ('NN', 'PL', 'APBM').

        Returns:
        - Updated local model after training.
        """
    ...
    def server_update(self, model_list, uploaded_weights, training_phase):
        """
        Aggregates client models using Federated Averaging (FedAvg) and updates the global model.

        Parameters:
        - model_list (list of nn.Module): List of client models.
        - uploaded_weights (list of float): Weights for each client model.
        - training_phase (str): The training phase ('NN', 'PL', 'APBM').
    ...
    def get_theta_init(self, model_init, train_loader_splitted, real_loc, show_figures=False, folder=None, mc_run=None):
        """
        Computes the initial estimate for theta using the trained NN model.

        Parameters:
        - model_init (nn.Module): The trained Neural Network model.
        - train_loader_splitted (list of DataLoader): List of DataLoaders for training.
        - real_loc (torch.Tensor): Ground truth jammer location.
        - show_figures (bool, optional): Whether to display the visualization.
        - folder (str, optional): Directory to save the figure.
        - mc_run (int, optional): Monte Carlo run identifier.

        Returns:
        - theta_init (nn.Parameter): Initial jammer location estimate.
        """
    ...
    def upload_weight(self, train_samples):
        """
        Computes and normalizes weights for federated averaging based on the number of training samples per client.

        Parameters:
        - train_samples (list): Number of samples for each client.

        Returns:
        - ws (torch.Tensor): Normalized weights.
        """
        ws = []
    ...
    def fedavg_NN(self, train_loader_splitted, test_loader, real_loc, show_figures, folder=None, mc_run=None):  
        """
        Trains the Neural Network (NN) model using Federated Learning and estimates an initial theta.

        Parameters:
        - train_loader_splitted (list of DataLoader): Training data loaders for each client.
        - test_loader (DataLoader): Global test dataset.
        - real_loc (torch.Tensor): Ground truth jammer location.
        - show_figures (bool): Whether to display visualization.
        - folder (str, optional): Directory to save results.
        - mc_run (int, optional): Monte Carlo run identifier.

        Returns:
        - Training and test losses over rounds, initial theta estimate, and updated global model.
        """
    ...
    def fedavg_PL(self, train_loader_splitted, test_loader, real_loc, show_figures, folder=None, mc_run=None):  
        """
        Trains the Pathloss Model (PL) using Federated Learning.
        ...
        Returns:
        - Training and test losses over rounds, final jammer location estimate, learned model parameters.
        """
        train_samples = torch.zeros(len(train_loader_splitted))
        for i in range(len(train_loader_splitted)):
            train_samples[i]=len(train_loader_splitted[i].dataset)
    ...
    def fedavg_APBM(self, train_loader_splitted, test_loader, real_loc, show_figures, folder=None, mc_run=None):
        """
        Trains the Augmented Physics-Based Model (APBM) using Federated Learning.
    ...
    def train_test_pipeline(self, train_loader_splitted, test_loader, real_loc, model_type, show_figures, folder=None, mc_run=None):  
        """
        Organizes the full federated training and testing pipeline for NN, PL, or APBM.

        Parameters:
        - train_loader_splitted (list of DataLoader): Training data loaders for each client.
        - test_loader (DataLoader): Global test dataset.
        - real_loc (torch.Tensor): Ground truth jammer location.
        - model_type (str): Model type ('PL' or 'APBM').
        - show_figures (bool): Whether to display visualization.
        - folder (str, optional): Directory to save results.
        - mc_run (int, optional): Monte Carlo run identifier.

        Returns:
        - Training and test losses for both NN initialization and PL/APBM training.
        - Initial and final jammer location estimates.
        - Learned model parameters and updated global model.
        """
    ...
    def evaluate(self, model, test_loader, nn_or_pl_or_apbm):
        """
        Evaluates a given model on the test dataset and computes the loss.

        Parameters:
        - model (nn.Module): The trained model.
        - test_loader (DataLoader): DataLoader for the test set.
        - nn_or_pl_or_apbm (str): Model type ('NN', 'PL', 'APBM').
    ...
    def get_learnt_parameters(self, model):
        """
        Retrieves the learned parameters (P0 and gamma) from the trained model.

        Parameters:
        - model (nn.Module): The trained model.

        Returns:
        - P0 (float): Learned P0 parameter.
        - gamma (float): Learned gamma parameter.
        """
        P0 = model.P0.item()
