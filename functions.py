...
def plot_field(test_y_predict,test_x,model_NN,trueJloc):
    sampGrid_x, sampGrid_mini, _ = getSampleGrid(test_y_predict,test_x, radius=500, Npoints=50)
    sampGrid_y = model_NN(sampGrid_x).detach()
    Jloc, err, _ = grid_peak_estimation(sampGrid_y,sampGrid_mini,trueJloc)  
    
    fig = go.Figure(data=[go.Scatter3d(x = sampGrid_x[:,0], y = sampGrid_x[:,1], z = sampGrid_y[:,0], mode='markers', marker=dict(
            size=3,
            #color = test_y_predict[:,0],                # set color to an array/list of desired values
            color = sampGrid_y[:,0],                # set color to an array/list of desired values
...
def estimate_param_rate2(reg_weights_rate_type,model_aug,model_aug_old,fixed_weight_rate = 0.1):
    # can be modified with linear prediction method given the set of past parameters
    
    w_rates_list = list()            
    for param_old,param in zip(model_aug_old.model_NN.parameters(),model_aug.model_NN.parameters()):       
        if reg_weights_rate_type == 'time_instants':
            param_rate = param-param_old
            DeltaT = 1 # time instnts elapsed (over whch rate is computed)(batches don't matter because the dataset is not related to a different jammer location)
        elif reg_weights_rate_type == 'fixed':
        ...
        # collect rates in iterable element
        w_rates_list.append(param_rate)
        
    return w_rates_list, DeltaT
