...
def main(fl_pw, grid_size, weight_ratio,data_name='R2',algorithm='FedAvg',\
    mu=0.1,local_epochs=1,epochs_init=50, model_mode='both',data_preprocesing=1,lr=0.005, test_mode=0,split='random'):
    tic = time.time()
    ...
    # Parameters
    ttSplit = 0.75 #3/4 #2/3 # train and test split ratio
    Nmc =1 # no. of Monte Carlo simulations runs
    fixed_data_select_size = 10000 # you get fixed_data_select_size training datapoints at each time
    ...
    layer_wid = [200,100,1]#[200,100,1]#[8,16,8,1] #[200,100,1] [64,16,8] #
    nonlinearity = 'tanh'#'tanh' #'tanh'
    epochs = 50
    # epochs_init = 100
    lr = lr #0.003
    # fl_pw = 1 # 0: no FL; 1: FL with pw
    ...
    data_sets_nan_r2 = [1, 4, 5, 6, 7, 11, 16, 19, 20,\
         21, 23, 24, 27, 28, 29, 30, 31,\
             35, 37, 40, 46, 48, 49]
                #12,13,14,18,25,32,33,34,38,42,43,44,45,47]
    ...
    # Xtest  = X[q:]; Ytest = Y[q:]
    # make a fake test dataset
    Xtest  = X[q:]; Ytest = Y[q:]
    # Xtest  = X[q-5:]; Ytest = Y[q-5:]
        ...
        for n in iter_list:
            ...
            for t in range(T):
                ...
                if t==0 or not(continual_learn):
                    # learning
                    if fl_pw:
                        train_loss, test_loss, model_NN = model_fl_pw.main_fl(train_x,
                                                                    train_y.reshape(-1,1),
                                                                    test_x,
                                                                    test_y.reshape(-1,1),
                                                                    layer_wid,nonlinearity,epochs_init,lr,
                                                                    data_max,data_min,grid_size, weight_ratio,
                                                                     algorithm,mu,local_epochs,model_mode,split)
                    else:
                ...
                if torch.isnan(train_loss[-1]) or torch.isinf(train_loss[-1]):# or torch.isnan(test_loss[-1])or torch.isinf(train_loss).any() or torch.isinf(test_loss).any():train_loss[-1]>=1000
                    print('Loss is nan at t: ',t)
                    nan_ts.append(t)
                    # break the loop and go to next n
                    continue
                ...
                if grad_estimation:
                    ...
                    Jloc = model_NN.get_theta().detach().cpu().clone().numpy()
                    err = Jloc-trueJloc[t,:]
    ...
    if fl_pw == 0:
        path = 'data/matlab/'+data_name+'_fl0'
    else:
        path = 'data/matlab/'+data_name+'_fl'+str(fl_pw)+str(grid_size)+weight_ratio+algorithm+model_mode
    if not os.path.exists(path):
        os.makedirs(path)

    io.savemat(path+'/rmsx.mat', mdict={'rmsx': err_nn_rmsx_snr})
    io.savemat(path+'/rmsy.mat', mdict={'rmsy': err_nn_rmsy_snr})
    # io.savemat(path+'/mle_rmsx.mat', mdict={'mle_rmsx': err_mle_rmsx_snr})
    # io.savemat(path+'/mle_rmsy.mat', mdict={'mle_rmsy': err_mle_rmsy_snr})
    # io.savemat(path+'/crbx.mat', mdict={'crbx': crb_varx_snr})
    # io.savemat(path+'/crby.mat', mdict={'crby': crb_vary_snr})

    io.savemat(path+'/meanx.mat', mdict={'meanx': nn_meanx_snr})
    io.savemat(path+'/meany.mat', mdict={'meany': nn_meany_snr})
    # io.savemat(path+'/mle_meanx.mat', mdict={'mle_meanx': mle_meanx_snr})
    # io.savemat(path+'/mle_meany.mat', mdict={'mle_meany': mle_meany_snr})
    io.savemat(path+'/truex.mat', mdict={'truex': true_Jlocx_snr})
    io.savemat(path+'/truey.mat', mdict={'truey': true_Jlocy_snr})

    io.savemat(path+'/snr.mat', mdict={'snr': snr_values_snr})
